...
export default {
  data() {
    return {
      ...
      emailStatus: '',
      jobFields: [
        'IT', '마케팅', '영업', '디자인', '연구개발', '교육', '건강', '금융', '법률', '엔지니어링', '제조업', '서비스업', '소매업', '문화예술', '공공기관', '스타트업', '기타'
      ],
      experienceLevels: ['신입', '1-2년', '3-5년', '6-10년', '10년 이상'],
      interviewStatuses: ['시작하지 않음', '진행 중', '거의 끝남'],
      targetCompanies: ['공무원', '공기업', '대기업', '중견/중소기업', '외국계 기업', '기타'],
      nameRules: [v => !!v || '이름을 입력해 주세요'],
      emailRules: [v => /.+@.+\..+/.test(v) || '유효한 이메일을 입력해 주세요'],
      passwordRules: [v => !!v || '비밀번호를 입력해 주세요'],
      phoneRules: [v => !v || /^[0-9]{10,11}$/.test(v) || '전화번호 형식이 올바르지 않습니다']
    };
  },
  methods: {
    async checkEmailDuplication() {
      if (this.email) {
        ...
        if (isDuplicate) {
          this.emailStatus = '중복됨';
        } else {
          this.emailStatus = '사용 가능';
        }
        this.isEmailChecked = true;
      }
    },
    submitForm() {
      if (this.valid && this.emailStatus === '사용 가능') {
        console.log({
          name: this.name,
          email: this.email,
          password: this.password,
          phone: this.phone,
          desiredJobField: this.desiredJobField,
          ...
          agreeToMarketing: this.agreeToMarketing
        });
      } else {
